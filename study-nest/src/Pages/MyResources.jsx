...
const RES_LIBRARY_API = "http://localhost/study-nest/src/api/ResourceLibrary.php"; // for share_recording action
...
export default function MyResources() {
    ...
    const tags = useMemo(
        () => [
            "All",
            ...uniq(
                rawItems.flatMap((x) => (x.tags || "").split(",").map((t) => t.trim()))
            ).filter(Boolean),
        ],
        [rawItems]
    );
    ...
    const filtered = useMemo(() => {
        ...
        if (sort === "New") list.sort((a, b) => +new Date(b.created_at || b.updated_at) - +new Date(a.created_at || a.updated_at));
        if (sort === "Top") list.sort((a, b) => (b.votes || 0) - (a.votes || 0));
        if (sort === "A-Z") list.sort((a, b) => String(a.title || "").localeCompare(String(b.title || "")));
    ...
    const deleteRecording = async (id) => {
        ...
        try {
            const res = await fetch(`${API_BASE}/recordings.php?id=${id}`, {
                method: "DELETE",
                credentials: "include",
            });
    ...
    return (
        <div className="min-h-screen bg-gradient-to-b from-slate-200 to-cyan-100 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 transition-all duration-500">
            ...
            <main className="pt-6 pb-10" style={{ paddingLeft: sidebarWidth, transition: "padding-left 300ms ease" }}>
                        ...
                        <div className="flex items-center justify-between gap-3">
                            <div>
                                ...
                                <p className="text-sm text-zinc-600 dark:text-zinc-400">
                                    Your personal uploads and room recordings. Share recordings to the Shared when you’re ready.
                                </p>
                            </div>
                            <Link
                                to="/resources"
...
function Card({ item, tab, onPreview, onDeleteRecording, onShareRecording, currentUser }) {
        ...
        <article className="group flex flex-col rounded-2xl bg-white shadow-md ring-1 ring-zinc-200 hover:shadow-lg transition dark:bg-slate-900 dark:ring-slate-800">
            ...
            <div className="flex-1 p-4">
                <div className="flex items-center gap-2">
                    <span className="truncate text-lg font-semibold text-zinc-900 dark:text-zinc-100" title={item.title}>
                        {item.title || "(Untitled)"}
                    </span>
                    {item.src_type === "file" && (
                ...
                <div className="mt-3 flex flex-wrap items-center gap-2 text-xs text-zinc-600 dark:text-zinc-400">
                    ...
                    {item.semester && <span className="rounded-full bg-zinc-100 px-2 py-0.5 dark:bg-slate-800">{item.semester}</span>}
                ...
                <div className="mt-2 flex flex-wrap gap-1">
                    {(item.tags || "")
                        ...
                        .filter(Boolean)
                        .map((t) => (
                            <span key={t} className="rounded-full border border-zinc-300 px-2 py-0.5 text-xs text-zinc-700 dark:text-zinc-200 hover:bg-zinc-100 dark:border-slate-700 dark:hover:bg-slate-800">
                                #{t}
                            </span>
                        ))}
                </div>
            ...
            <div className="flex items-center justify-between border-t border-zinc-100 dark:border-slate-800 px-4 py-3 text-xs">
                <div className="text-zinc-500 dark:text-zinc-400">
                    {safeDate(item.created_at || item.updated_at)}
                </div>
                <div className="flex items-center gap-2">
                    {isRecording && isOwner && (
                        <>
...
function PreviewModal({ file, onClose }) {
    ...
    return (
        <div className="fixed inset-0 z-40 bg-black/70 p-4" onClick={onClose}>
            <div className="mx-auto max-w-5xl rounded-2xl bg-white p-4 shadow-2xl dark:bg-slate-900" onClick={(e) => e.stopPropagation()}>
                <div className="flex items-center justify-between">
                    <h3 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100 truncate">{file.name}</h3>
                    <button onClick={onClose} className="rounded-md p-2 text-zinc-500 hover:bg-zinc-100 dark:hover:bg-slate-800" aria-label="Close">
                        <XIcon className="h-5 w-5" />
                    </button>
                </div>
                <div className="mt-3">
                    {isImage ? (
                        <img src={file.url} alt={file.name} className="max-h-[70vh] w-full object-contain rounded-lg ring-1 ring-zinc-200 dark:ring-slate-800" />
                    ) : isPdf ? (
                        <iframe title="preview" src={file.url} className="h-[70vh] w-full rounded-lg ring-1 ring-zinc-200 dark:ring-slate-800" />
                    ) : (
                        <div className="grid place-items-center rounded-lg border border-dashed border-zinc-300 dark:border-slate-700 p-10 text-center text-sm text-zinc-600 dark:text-zinc-300">
                            ...
                            <a href={file.url} download className="mt-3 inline-flex rounded-xl bg-zinc-900 px-4 py-2 font-semibold text-white hover:bg-zinc-800 dark:bg-slate-800 dark:hover:bg-slate-700">
                                Download
                            </a>
                        </div>
                    )}
